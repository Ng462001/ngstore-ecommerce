...
const ReturnTracking = ({ request, isFullPage = false }) => {
    ...
    const getSteps = () => {
        if (request.type === 'Return') {
            return ['Pending', 'Approved', 'Pickup Scheduled', 'Received', 'Refund Processed']
        } else {
            return ['Pending', 'Approved', 'Pickup Scheduled', 'Received', 'Exchange Shipped']
        }
    }
    ...
    const getCurrentStep = () => {
        switch (request.status) {
            case 'Completed': return 4 // Refund processed or exchange shipped
    ...
    const getStatusDate = (status) => {
        ...
        const update = request.statusUpdates.find(u => u.status === status)
        if (update && update.timestamp) {
            return format(new Date(update.timestamp), 'EEEE, MMMM d, yyyy \'at\' h:mm a')
        }
        return null
    }
    ...
    const getStepContent = (stepIndex) => {
        ...
        const stepDate = getStatusDate(stepStatus) || (stepIndex === 0 ? format(new Date(request.createdAt), 'EEEE, MMMM d, yyyy \'at\' h:mm a') : 'Awaiting update')
        ...
        const stepContents = {
            ...
            'Exchange Shipped': {
                ...
                description: 'Your exchange item has been shipped and is on its way to you.',
                date: stepDate,
                icon: <ShippingIcon />
            }
        }
        ...
        <Container maxWidth={isFullPage ? "lg" : false} sx={{ p: 0 }}>
            <Paper elevation={0} sx={{ bgcolor: 'grey.50', borderRadius: 2, p: 4 }}>
                ...
                <Typography variant="body2" sx={{ color: 'text.secondary', mb: 4 }}>
                    Request #{request._id?.toUpperCase()} • Submitted on {format(new Date(request.createdAt), 'MMMM d, yyyy')}
                </Typography>
                                    ...
                                    <CardContent>
                                        ...
                                        <Box sx={{ mt: 2 }}>
                                            <Typography variant="body1" paragraph>
                                                <strong>{request.pickupAddress?.street || 'N/A'}</strong>
                                            </Typography>
                                            <Typography variant="body2" color="text.secondary" paragraph>
                                                {request.pickupAddress?.city || ''} - {request.pickupAddress?.zipCode || ''}<br />
                                                {request.pickupAddress?.state || ''}<br />
                            ...
                            <Grid item xs={12} md={6}>
                                <Paper sx={{ bgcolor: 'info.light', height: '100%', p: 3, borderRadius: 2, border: 1, borderColor: 'info.main' }}>
                                    ...
                                    <Typography variant="body2" sx={{ color: 'info.dark', mb: 2 }}>
                                        For any questions about your {request.type.toLowerCase()} request, contact our customer support.
                                    </Typography>
