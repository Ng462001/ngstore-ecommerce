              ...
              <Box sx={{ mt: 3 }}>
                ...
                >
                  ...
                  <Chip
                    ...
                    size="small"
                    color={
                      order.paymentMethod === "COD" ? "warning" : "success"
                    }
                  />
                </Box>
...
const CustomerDetailsModal = ({
  ...
  const fetchCustomerOrders = async () => {
    ...
    try {
      ...
      try {
        const response = await axios.get(
          `${API_URL}/admin/users/${customer._id}/orders`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
      ...
      if (orders.length > 0) {
        ...
        const orderDates = orders.map(
          (o) => new Date(o.createdAt || Date.now()),
        );
        const lastOrderDate =
  ...
  const renderOverviewTab = () => {
    ...
    const displayCustomer = editMode
      ? editedCustomer
      : customerDetails || customer;

    if (!displayCustomer) {
      return <Alert severity="warning">Customer data not available</Alert>;
    }
          ...
          <Grid item xs={12} md={4}>
            <Paper sx={{ p: 3, borderRadius: 2, height: "100%" }}>
              ...
              <Box sx={{ textAlign: "center", mb: 3 }}>
                ...
                <Chip
                  icon={
                    displayCustomer.role === "admin" ? <Security /> : <Person />
                  }
                    ...
                    secondary={
                      <Box component="span">
                        ...
                        {displayCustomer.createdAt && (
                          ...
                          >
                            {formatDistanceToNow(
                              new Date(displayCustomer.createdAt),
                              { addSuffix: true },
                            )}
                          </Typography>
                        )}
                ...
                {editMode ? (
                  ...
                  <Paper
                    ...
                    sx={{
                      ...
                      bgcolor: "grey.50",
                      minHeight: 80,
                      fontStyle: displayCustomer.adminNotes
                        ? "normal"
                        : "italic",
                      color: displayCustomer.adminNotes
                        ? "text.primary"
                        : "text.secondary",
                    }}
                  >
                  ...
                  {displayCustomer.shippingAddress ? (
                    <Box>
                      <Typography variant="body1" gutterBottom>
                        <strong>
                          {displayCustomer.shippingAddress.fullName ||
                            displayCustomer.name ||
                            "No Name"}
                        </strong>
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                        {displayCustomer.shippingAddress.address ||
                          "No address"}
                      </Typography>
                      <Typography variant="body2" color="text.secondary">
                      ...
                      >
                        ...
                        />
                        {displayCustomer.shippingAddress.phone ||
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={6} md={3}>
                ...
                <Typography variant="h5">
                  {orderStats.totalOrders
                    ? (
                        (orderStats.successfulOrders / orderStats.totalOrders) *
                        100
                      ).toFixed(1)
                    : 0}
                  %
              ...
              <Grid item xs={6} md={3}>
                ...
                <Typography variant="h5">
                  {orderStats.totalOrders
                    ? (
                        (orderStats.cancelledOrders / orderStats.totalOrders) *
                        100
                      ).toFixed(1)
                    : 0}
                  %
                          ...
                          <Box>
                            {(order.orderItems || []).map((item, idx) => (
                              <Typography key={idx} variant="body2">
                                {item.quantity || 0}x {item.name || "Unknown"} -{" "}
      ...
      >
        ...
        >
          <Tabs value={activeTab} onChange={handleTabChange}>
            ...
            <Tab
              label={`Orders (${orderStats.totalOrders})`}
              icon={<ShoppingBag />}
              iconPosition="start"
            />
          </Tabs>
