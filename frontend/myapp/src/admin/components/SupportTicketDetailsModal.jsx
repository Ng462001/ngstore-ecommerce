...
const SupportTicketDetailsModal = ({ open, onClose, ticket, onTicketUpdate, onSendResponse }) => {
    ...
    const getStatusColor = (status) => {
        const statusMap = {
            ...
            'In Progress': { color: 'warning', bgColor: '#fff3e0', textColor: '#f57c00' },
    ...
    const getPriorityColor = (priority) => {
        const priorityMap = {
            'Low': { color: 'success', bgColor: '#e8f5e9', textColor: '#2e7d32', icon: <ArrowDownward /> },
            'Medium': { color: 'info', bgColor: '#e3f2fd', textColor: '#1565c0', icon: <Remove /> },
            'High': { color: 'warning', bgColor: '#fff3e0', textColor: '#f57c00', icon: <ArrowUpward /> },
            'Urgent': { color: 'error', bgColor: '#ffebee', textColor: '#c62828', icon: <PriorityHigh /> }
        };
    ...
    const calculateResponseTime = () => {
        ...
        const firstResponse = ticket.responses.find(r => r.senderRole === 'Admin');
        if (!firstResponse) return 'No response yet';

        const createdAt = new Date(ticket.createdAt);
        const firstResponseTime = new Date(firstResponse.createdAt);
        const diffHours = Math.abs(firstResponseTime - createdAt) / (1000 * 60 * 60);
        ...
        return (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <Grid container spacing={3}>
                    {/* Conversation Thread */}
                    <Grid item xs={12} md={8}>
                        <Paper sx={{ p: 3, borderRadius: 2, height: '100%', minHeight: 500 }}>
                                        ...
                                        {ticket.responses.map((response, index) => (
                                            ...
                                            >
                                                <Box
                                                    sx={{
                                                        display: 'flex',
                                                        flexDirection: response.senderRole === 'Admin' ? 'row-reverse' : 'row',
                                                ...
                                                >
                                                    <Avatar
                                                        sx={{
                                                            width: 40,
                                                            height: 40,
                                                            bgcolor: response.senderRole === 'Admin' ? 'primary.main' : 'grey.500'
                                                        }}
                                                    >
                                                        {response.senderRole === 'Admin' ? 'A' : ticket.user?.name?.charAt(0).toUpperCase() || 'U'}
                                                    </Avatar>
                                                    <Card
                                                        sx={{
                                                            maxWidth: '70%',
                                                            bgcolor: response.senderRole === 'Admin' ? 'primary.light' : 'grey.100',
                                                            borderTopLeftRadius: response.senderRole === 'Admin' ? 12 : 2,
                                                            borderTopRightRadius: response.senderRole === 'Admin' ? 2 : 12,
                                                            p: 2
                                                        }}
                                                    >
                                                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                                                            <Typography variant="subtitle2" fontWeight="bold">
                                                                {response.senderRole === 'Admin' ? 'Support Agent' : ticket.user?.name || 'Customer'}
                                                            </Typography>
                            ...
                            <Paper sx={{ p: 3, borderRadius: 2 }}>
                                ...
                                <Stack spacing={1}>
                                    <Button
                                        ...
                                        fullWidth
                                        onClick={() => onTicketUpdate && onTicketUpdate(ticket._id, { status: 'Resolved' })}
                                    ...
                                    <Button
                                        ...
                                        startIcon={<Cancel />}
                                        fullWidth
                                        onClick={() => onTicketUpdate && onTicketUpdate(ticket._id, { status: 'Closed' })}
                                    ...
                                    <Button
                                        ...
                                        startIcon={<Refresh />}
                                        fullWidth
                                        onClick={() => onTicketUpdate && onTicketUpdate(ticket._id, { status: 'In Progress' })}
                                                ...
                                                <Chip
                                                    ...
                                                    color={
                                                        item.action === 'Created' ? 'primary' :
                                                            item.action === 'Response' ? 'info' : 'default'
                                                    }
                                                />
                                            </TableCell>
                                            <TableCell>
        ...
        >
            ...
            <Box sx={{ borderBottom: 1, borderColor: 'divider', px: 3, bgcolor: 'white' }}>
                <Tabs value={activeTab} onChange={handleTabChange}>
                    ...
                    <Tab label={`Conversation (${ticket.responses?.length || 0})`} icon={<Chat />} iconPosition="start" />
