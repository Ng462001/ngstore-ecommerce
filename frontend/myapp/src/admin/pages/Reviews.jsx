...
const Reviews = () => {
  ...
  const fetchStats = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/admin/reviews/stats`,
        {
          headers: {
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}/api/admin/reviews/products`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(
        `${import.meta.env.VITE_API_URL}/api/admin/reviews/${reviewToDelete}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
  ...
  const handleUpdateReview = async () => {
    ...
    try {
      await axios.put(
        `${import.meta.env.VITE_API_URL}/api/admin/reviews/${selectedReview._id}`,
        editReviewData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  return (
    <div className="space-y-6">
                        ...
                        reviews.map((review) => (
                          <TableRow key={review._id} hover>
                              ...
                              <Box display="flex" alignItems="center">
                                <Avatar
                                  src={
                                    ...
                                    review.productImage.startsWith("http")
                                      ? review.productImage
                                      : `${import.meta.env.VITE_API_URL}${review.productImage || ""}`
                                  }
                                ...
                                <Box>
                                  ...
                                  >
                                    ...
                                    {formatCurrency(review.productPrice)}
                                  </Typography>
                                </Box>
                              </Box>
                            </TableCell>
                            ...
                            <TableCell>
                              ...
                              >
                                {review.date
                                  ? format(new Date(review.date), "hh:mm a")
                                  : ""}
                              </Typography>
                            </TableCell>
                            <TableCell align="center">
      ...
      >
        {selectedReview && (
          <>
            <DialogTitle className="font-bold">Review Details</DialogTitle>
            <DialogContent>
                  ...
                  <Box className="space-y-3">
                    ...
                    <div>
                      ...
                      <Typography variant="body1" className="mt-1">
                        {selectedReview.date
                          ? format(new Date(selectedReview.date), "PPP")
                          : "N/A"}
                      </Typography>
                      <Typography variant="body2" className="text-gray-500">
                        {selectedReview.date
                          ? format(new Date(selectedReview.date), "hh:mm:ss aa")
                          : ""}
                      </Typography>
                    </div>
                  </Box>
      ...
      >
        <DialogTitle className="font-bold">Delete Review</DialogTitle>
        <DialogContent>
          <Typography>
