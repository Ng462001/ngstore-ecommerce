...
const AdminOrderDetails = () => {
  ...
  const statusTimeline = useMemo(() => {
    ...
    if (order.status === "Returned") {
      return [
        ...defaultSteps,
        { status: "Returned", action: "Order Returned", isError: true },
      ];
    }
    ...
    if (order.status === "Refunded") {
      return [
        ...defaultSteps,
        { status: "Refunded", action: "Order Refunded" },
      ];
    }
  ...
  const fetchActivityLog = useCallback(async () => {
    try {
      ...
      const response = await axios.get(
        `${API_URL}/api/admin/orders/${id}/activity`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
  ...
  const handleTogglePriority = async () => {
    try {
      ...
      showSnackbar(
        `Order ${newPriorityStatus ? "marked as priority" : "removed from priority"}`,
        "success",
      );
    } catch (err) {
  ...
  const getActiveStep = () => {
    if (!order) return 0;
    const index = statusTimeline.findIndex(
          ...
          <Stepper activeStep={getActiveStep()} orientation="horizontal">
            {statusTimeline.map((step, index) => (
              <Step
                key={step.status}
                completed={
                  index < getActiveStep() || order.status === "Delivered"
                }
              >
                <StepLabel
                  error={step.isError}
                  optional={
                    index === getActiveStep() ? (
                      <Typography
                        variant="caption"
                        color={step.isError ? "error" : "primary"}
                      >
                        {step.status}
                      </Typography>
                    ) : null
            ...
            >
              <Paper sx={{ p: 3 }}>
                ...
                <Stack spacing={2}>
                  ...
                  {customerStats &&
                    (customerStats.totalOrders || customerStats.totalSpent) && (
                      <>
                        <Divider />
                        <Box>
                          <Typography
            ...
            >
              <Paper sx={{ p: 3 }}>
                ...
                {order.shippingAddress ? (
                  ...
                  <Alert severity="info">No shipping address provided</Alert>
                )}
              </Paper>
            </motion.div>
                ...
                {statusOptions
                  .filter(
                    (option) =>
                      ...
                      getAllowedTransitions(order?.status).includes(
                        option.value,
                      ),
                  )
                  .map((option) => (
      ...
      >
          ...
          <Stack spacing={3} sx={{ mt: 1 }}>
                ...
                {paymentStatusOptions
                  .filter(
                    (option) =>
                      ...
                      getAllowedPaymentTransitions(order?.paymentStatus)
                        .map((s) => s.toLowerCase())
            ...
            {selectedPaymentStatus === "refunded" && (
              <Alert severity="warning">
                Marking as refunded will also update order status to "Refunded"
                and reverse inventory.
              </Alert>
            )}
          </Stack>
        </DialogContent>
        <DialogActions>
          ...
          <Button
            ...
            color="primary"
            disabled={
              loadingAction || selectedPaymentStatus === order?.paymentStatus
            }
            startIcon={loadingAction ? <CircularProgress size={20} /> : null}
          >
          ...
          <Stack spacing={3} sx={{ mt: 1 }}>
            ...
            <Box sx={{ display: "flex", gap: 1, alignItems: "flex-start" }}>
              ...
              <Button
                ...
                startIcon={
                  loadingAction ? <CircularProgress size={20} /> : <Add />
                }
                sx={{ mt: 1 }}
              >
                Add
