...
const Home = () => {
  ...
  const fetchHomeProducts = async () => {
    try {
      ...
      const [latestRes, trendingRes] = await Promise.all([
        fetch(
          `${import.meta.env.VITE_API_URL}/api/products?limit=8&sort=createdAt&order=desc`,
        ),
        fetch(
          `${import.meta.env.VITE_API_URL}/api/products?limit=8&sort=rating&order=desc`,
        ),
      ]);
  ...
  const getProductPriceInfo = (product) => {
    ...
    if (numDiscountedPrice > 0 && numDiscountedPrice < numPrice) {
      ...
      numDiscountedPrice > 0 &&
      numOriginalPrice > numDiscountedPrice
    ) {
      displayPrice = numDiscountedPrice;
      originalPrice = numOriginalPrice;
    } else {
      ...
      >
        ...
        <CardContent sx={{ p: 2 }}>
          ...
          <Skeleton
            ...
            height={16}
            width="70%"
            sx={{ borderRadius: 1, bgcolor: "grey.200", mt: 1 }}
          />
