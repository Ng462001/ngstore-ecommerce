...
const colorSchema = new mongoose.Schema(
  {
    ...
    class: {
      ...
      validate: {
        ...
        message:
          "Color class must be a valid Tailwind CSS background class (e.g. bg-red-500)",
      },
    },
  },
  { _id: false },
...
const sizeSchema = new mongoose.Schema(
  {
    name: {
      ...
      enum: {
        ...
        message:
          "Size name must be one of: XXS, XS, S, M, L, XL, 2XL, 3XL, 4XL",
      },
    },
    inStock: {
      type: Boolean,
...
// Rating & Review virtual metrics
productSchema.virtual("ratingCount").get(function () {
  return this.rating?.count || (this.reviews ? this.reviews.length : 0);
});

productSchema.virtual("reviewCount").get(function () {
  if (this.rating?.reviewCount !== undefined) {
...
productSchema.pre("validate", function (next) {
  ...
  if (this.originalPrice && this.discountedPrice) {
    ...
    if (this.discount && Math.abs(calculatedDiscount - this.discount) > 1) {
      return next(
        new Error(
          `Discount percentage mismatch. Calculated: ${calculatedDiscount}%, Provided: ${this.discount}%`,
        ),
      );
    }
  }
  ...
  if (this.reviews && Array.isArray(this.reviews) && this.reviews.length > 0) {
    ...
    if (validReviews.length > 0) {
      ...
      const writtenCount = validReviews.filter(
        (r) =>
          r.comment &&
          typeof r.comment === "string" &&
          r.comment.trim().length > 0,
      ...
      this.rating = {
        ...
        average: Math.min(
          5,
          Math.max(0, Number((total / validReviews.length).toFixed(1))),
        ),
        breakdown,
      };
    } else {
...
productSchema.statics.searchProducts = function (query, filters = {}) {
  ...
  if (filters.minPrice || filters.maxPrice) {
    ...
    if (filters.maxPrice)
      searchCriteria.price.$lte = parseFloat(filters.maxPrice);
  }
