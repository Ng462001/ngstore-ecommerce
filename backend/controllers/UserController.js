...
class UserController {
  ...
  static getUserReviews = async (req, res) => {
    try {
      ...
      const reviewedProductIds = new Set(
        givenReviews.map((r) => r.productId.toString()),
      );
      ...
      const pendingReviews = pendingProductIds.map((id) => {
        ...
        const prodDoc = pendingProductsDocs.find(
          (p) => p._id.toString() === id,
        );
        return {
          productId: id,
          ...
          productImage:
            prodDoc?.mainImage || prodDoc?.image || orderInfo.productImage,
  ...
  static getRecentSearches = async (req, res) => {
    try {
  ...
  // Add / Upsert a Recent Search query in User database
  static addRecentSearch = async (req, res) => {
    try {
      ...
      if (!clean || clean.length < 2) {
        return res
          .status(400)
          .json({ success: false, message: "Search query is too short" });
      }
      ...
      const existing = Array.isArray(user.recentSearches)
        ? user.recentSearches
        : [];
      const filtered = existing.filter(
        (item) => item.toLowerCase() !== clean.toLowerCase(),
      );
      user.recentSearches = [clean, ...filtered].slice(0, 10);
      await user.save({ validateBeforeSave: false });
  ...
  // Remove a single query from user's recent searches
  static deleteRecentSearch = async (req, res) => {
    try {
      const { query } = req.body;
      const user = await User.findById(req.user._id);
      if (!user) {
  ...
  // Clear all recent searches for user in database
  static clearAllRecentSearches = async (req, res) => {
    try {
      const user = await User.findById(req.user._id);
      if (!user) {
        return res
